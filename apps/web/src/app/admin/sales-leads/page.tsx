...
import { Lead, LeadDetail, Stats, EmailTemplate, CampaignPreview } from './types';
import { API_BASE, STATE_OPTIONS, TEMPLATE_ICONS, TEMPLATE_COLORS, STATUS_CONFIG, PRIORITY_COLORS } from './constants';
      ...
      <main className="flex-1 p-6 overflow-y-auto">
        ...
        <div className="bg-white border border-slate-200 rounded-lg p-4 mb-4">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            >
              {STATE_OPTIONS.map(s => (
                <option key={s.value} value={s.value} disabled={!!(s as any).disabled}>{s.label}</option>
              ))}
            </select>
            <select
