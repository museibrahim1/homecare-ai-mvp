...
export default function QuickSetupPage() {
  ...
  return (
    <div className="landing-dark flex min-h-screen bg-[#0a0a1a]">
      <Sidebar />
          ...
          {result && (
            <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-2xl p-6 mb-8">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h2 className="text-lg font-semibold text-emerald-300">Agency Created Successfully</h2>
              </div>
              ...
              <div className="space-y-3 mb-6">
                ...
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span className="text-white font-mono text-xs">{result.business_id}</span>
                </div>
              </div>

              <div className="bg-[#0a0a1a]/60 rounded-xl p-4 mb-4">
                <p className="text-xs text-gray-400 uppercase tracking-wider mb-3 font-semibold">Login Credentials</p>
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    <span className="text-gray-400 text-sm">Email</span>
                    <span className="text-white font-mono text-sm">{ownerEmail}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-gray-400 text-sm">Password</span>
                    <span className="text-amber-300 font-mono text-sm">{result.temporary_password}</span>
                  </div>
                </div>
              </div>
          ...
          {!result && (
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="bg-[#111128] border border-white/[0.06] rounded-2xl p-6 space-y-5">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white placeholder:text-gray-600 focus:outline-none focus:border-primary-500/50 focus:ring-1 focus:ring-primary-500/30 text-sm"
                  />
                </div>

                {/* Owner Name */}
                <div>
                  <label className="block text-sm text-gray-400 mb-1.5">Owner Name *</label>
                  <input
                    type="text"
                    required
                    value={ownerName}
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white focus:outline-none focus:border-primary-500/50 focus:ring-1 focus:ring-primary-500/30 text-sm appearance-none"
                  >
                    <option value="" className="bg-[#111128]">Select state...</option>
                    {US_STATES.map(s => (
                      <option key={s} value={s} className="bg-[#111128]">
                        {STATE_NAMES[s]} ({s})
                      </option>
                    ))}
                  </select>
              ...
              <div className="bg-[#111128] border border-white/[0.06] rounded-2xl p-6 space-y-5">
                ...
                {/* Services */}
                <div>
                  <label className="block text-sm text-gray-400 mb-3">Services Offered</label>
                ...
                {/* Estimated Clients */}
                <div>
                  <label className="block text-sm text-gray-400 mb-1.5">Estimated Clients <span className="text-gray-600">(optional)</span></label>
                  <select
                    value={estimatedClients ?? ''}
                    onChange={e => setEstimatedClients(e.target.value ? Number(e.target.value) : null)}
                    className="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white focus:outline-none focus:border-primary-500/50 focus:ring-1 focus:ring-primary-500/30 text-sm appearance-none"
                  >
                    <option value="" className="bg-[#111128]">Select range...</option>
                    {CLIENT_RANGES.map(r => (
                      <option key={r.value} value={r.value} className="bg-[#111128]">
                        {r.label}
                      </option>
                    ))}
                  </select>
