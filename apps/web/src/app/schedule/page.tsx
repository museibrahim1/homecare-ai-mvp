    ...
    <>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        {[
          ...
          { label: 'Google Sync', value: googleConnected ? 'On' : 'Off', icon: Link2, color: googleConnected ? 'text-emerald-600' : 'text-slate-500', bg: googleConnected ? 'bg-emerald-50' : 'bg-slate-100', show: true },
        ].filter(s => s.show).map((s, i) => (
          <div
            key={i}
            className={`glass-card p-3 lg:p-4 flex items-center gap-3 ${s.label === 'Google Sync' ? 'cursor-pointer hover:bg-white/80' : ''}`}
            onClick={s.label === 'Google Sync' ? () => setShowConnectModal(true) : undefined}
            role={s.label === 'Google Sync' ? 'button' : undefined}
          >
      ...
      {showConnectModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-[#10211F]/40 backdrop-blur-sm" onClick={() => setShowConnectModal(false)} />
          <div className="relative glass-card w-full max-w-sm overflow-hidden !rounded-2xl">
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#FFFFFFE0]">
              <h2 className="text-base font-semibold text-[#10211F]">Google Calendar</h2>
              <button type="button" onClick={() => setShowConnectModal(false)} className="p-1.5 text-slate-500 hover:text-[#10211F] hover:bg-white/60 rounded-lg transition-colors"><X className="w-4 h-4" /></button>
            </div>
            <div className="p-5">
              {googleConnected ? (
                <>
                  <div className="flex items-center gap-3 mb-4 p-3 bg-emerald-50 border border-emerald-200 rounded-xl">
                    ...
                    <div><p className="text-[#10211F] text-sm font-medium">Connected</p><p className="text-[11px] text-slate-500">Events sync automatically</p></div>
                  </div>
                  <button type="button" onClick={handleDisconnectGoogle} className="w-full px-4 py-2 text-sm bg-red-50 text-red-600 border border-red-200 rounded-lg hover:bg-red-100 transition-colors">Disconnect</button>
                </>
              ) : (
                <>
                  <div className="text-center mb-5">
                    <div className="w-12 h-12 bg-[#0D94881A] rounded-xl flex items-center justify-center mx-auto mb-3">
                      <CalendarDays className="w-6 h-6 text-primary-500" />
                    </div>
                    <p className="text-[#4B6B66] text-sm mb-1">Sync with Google Calendar</p>
                    <p className="text-[11px] text-slate-400">Appointments sync to all your devices</p>
                  </div>
                  <button type="button" onClick={handleConnectGoogle} className="w-full px-4 py-2.5 bg-white text-[#10211F] border border-[#FFFFFFE0] rounded-lg hover:bg-slate-50 transition-colors font-medium text-sm flex items-center justify-center gap-2">
