        ...
        <div className="flex-1 p-4 lg:p-8">
          <div className="max-w-6xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
              <div>
                <h1 className="text-xl lg:text-2xl font-bold text-dark-50">Billing & Plans</h1>
                <p className="text-dark-400 text-sm mt-0.5">Manage your subscription and payment methods</p>
              </div>
              {subscription?.stripe_customer_id && (
                <button
                  onClick={handleManageBilling}
                  disabled={portalLoading}
                  className="flex items-center gap-2 px-4 py-2 bg-dark-800 border border-dark-700 rounded-lg text-sm font-medium text-dark-200 hover:bg-dark-700/30 transition-colors"
                >
            ...
            {subscription && plan && (
              <div className="bg-dark-800 border border-dark-700 rounded-lg p-5 mb-6">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2.5">
                        <h2 className="text-lg font-semibold text-dark-50">{plan.name} Plan</h2>
                ...
                >
                  ...
                  {tab.count !== undefined && (
                    <span className={`ml-1.5 text-[10px] px-1.5 py-0.5 rounded-full ${
                      activeTab === tab.id ? 'bg-primary-50 text-primary-600' : 'bg-dark-700 text-dark-400'
                    }`}>
                      {tab.count}
                    </span>
                  )}
            ...
            {activeTab === 'plans' && (
              <div>
                ...
                <div className="flex items-center justify-between mb-6">
                  <p className="text-sm text-dark-400">Select a plan that fits your agency</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  {allPlans.map((p) => {
                    ...
                    return (
                      <div
                        key={p.id}
                        className={`relative bg-dark-800 border rounded-lg overflow-hidden transition-all ${
                          isCurrentPlan
                        ...
                        <div className="p-5">
                          <div className="flex items-center gap-2 mb-3">
                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                              isCurrentPlan ? 'bg-primary-50' : 'bg-dark-900'
                            }`}>
                              <TierIcon className={`w-4 h-4 ${isCurrentPlan ? 'text-primary-600' : 'text-dark-400'}`} />
                            </div>
                            <h3 className="font-semibold text-dark-50">{p.name}</h3>
                          </div>

                          <p className="text-dark-400 text-xs mb-4 min-h-[32px]">
                          ...
                          {p.is_contact_sales ? (
                            <div className="mb-5">
                              <span className="text-2xl font-bold text-dark-50">Custom</span>
                              <p className="text-dark-400 text-xs mt-0.5">Tailored to your needs</p>
                            </div>
                          ) : (
                            <div className="mb-5">
                              <div className="flex items-baseline gap-1">
                                <span className="text-3xl font-bold text-dark-50">{formatCurrency(price)}</span>
                                <span className="text-dark-400 text-sm">/mo</span>
                              </div>
                              ...
                              {p.setup_fee > 0 && (
                                <p className="text-dark-400 text-xs mt-0.5">+ {formatCurrency(p.setup_fee)} setup fee</p>
                              )}
                            </div>
                          )}
                          ...
                          {isCurrentPlan ? (
                            ...
                            <a
                              ...
                              className="w-full flex items-center justify-center gap-2 py-2.5 text-sm font-medium text-dark-200 bg-dark-900 hover:bg-dark-700/50 rounded-lg border border-dark-700 transition-colors mb-5"
                            >
                  ...
                  {allPlans.length === 0 && (
                    <div className="col-span-full bg-dark-800 border border-dark-700 rounded-lg p-12 text-center">
                      <CreditCard className="w-10 h-10 text-slate-300 mx-auto mb-3" />
                      <h3 className="text-lg font-semibold text-dark-50 mb-1">No plans available</h3>
                      <p className="text-dark-400 text-sm">Plans are being configured. Contact support for access.</p>
                    </div>
                  )}
                </div>

                {/* Feature Comparison */}
                <div className="mt-8 bg-dark-800 border border-dark-700 rounded-lg overflow-hidden">
                  <div className="px-5 py-4 border-b border-dark-700">
                    <h3 className="text-sm font-semibold text-dark-50">All Plans Include</h3>
                  </div>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-dark-700">
                    {[
                      ...
                      <div key={label} className="bg-dark-800 p-4 flex items-center gap-3">
              ...
              <div className="space-y-4">
                {plan ? (
                  <>
                    ...
                    <div className="bg-dark-800 border border-dark-700 rounded-lg p-5">
                      <div className="flex items-center gap-2.5 mb-4">
                        ...
                        <h3 className="text-sm font-semibold text-dark-50">Payment Method</h3>
                      </div>
                      {subscription?.stripe_customer_id ? (
                        <div className="flex items-center justify-between p-3 bg-dark-900 rounded-lg border border-dark-700">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-sm font-medium text-dark-100">Card on file</p>
                              <p className="text-xs text-dark-400">Managed via Stripe</p>
                            </div>
                          </div>
                          <button
                            onClick={handleManageBilling}
                        ...
                        <div className="text-center py-6">
                          ...
                          <p className="text-dark-400 text-sm">No payment method on file</p>
                          <p className="text-dark-400 text-xs mt-0.5">Subscribe to a plan to add one</p>
                        </div>
                      )}
                    </div>
                  </>
                  ...
                  <div className="bg-dark-800 border border-dark-700 rounded-lg p-12 text-center">
                    ...
                    <h3 className="text-lg font-semibold text-dark-50 mb-1">No active subscription</h3>
                    <p className="text-dark-400 text-sm mb-4">Subscribe to a plan to see your usage</p>
                    <button
              ...
              <div>
                {invoices.length > 0 ? (
                      ...
                      <tbody>
                        {invoices.map((inv, i) => (
                          <tr key={inv.id} className={i % 2 === 1 ? 'row-alt' : ''}>
                            <td className="font-medium text-dark-50">{inv.invoice_number || `INV-${i + 1}`}</td>
                            <td className="text-dark-300">{formatDate(inv.invoice_date)}</td>
                            <td className="font-medium text-dark-50">{formatCurrency(inv.amount)}</td>
                            <td>
                  ...
                  <div className="bg-dark-800 border border-dark-700 rounded-lg p-12 text-center">
                    ...
                    <h3 className="text-lg font-semibold text-dark-50 mb-1">No invoices yet</h3>
                    <p className="text-dark-400 text-sm">Invoices will appear here after your first payment</p>
                  </div>
                )}
              </div>
            )}
...
function UsageMeter({ icon: Icon, label, used, limit, unit, percent }: {
  ...
  return (
    <div className="bg-dark-800 border border-dark-700 rounded-lg p-5">
      ...
      <p className="text-[11px] text-dark-400 mt-1.5">{percent}% used</p>
    </div>
  );
}
