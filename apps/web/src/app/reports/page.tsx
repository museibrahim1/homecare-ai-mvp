...
export default function ReportsPage() {
  ...
  const getServiceColor = (type: string) => {
    ...
    return colors[type] || 'bg-dark-700 text-dark-300 border-dark-600';
  };
      ...
      case 'timesheet':
        return (
          <div className="p-6 space-y-6">
            ...
            <div>
              <h4 className="text-dark-50 font-medium mb-3">Services Breakdown</h4>
            ...
            {reportData.assessments?.length > 0 && (
              <div>
                <h4 className="text-dark-50 font-medium mb-3">Recent Assessments</h4>
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  {reportData.assessments.map((a: any, i: number) => (
                    <div 
                      key={i} 
                      onClick={() => router.push(`/visits/${a.id}`)}
                      className="flex items-center justify-between p-3 bg-dark-900/30 rounded-lg cursor-pointer hover:bg-dark-700/50 transition"
                    >
                      <div>
                        <p className="text-dark-50 text-sm font-medium">{a.client_name}</p>
        ...
        return (
          <div className="p-6 space-y-6">
            ...
            <div className="bg-gradient-to-r from-accent-green/10 to-primary-500/10 rounded-xl p-4 border border-accent-green/20">
              <h4 className="text-dark-50 font-bold text-lg">{reportData.month} {reportData.year}</h4>
            ...
            <div className="grid grid-cols-4 gap-3">
              <div className="bg-dark-700 rounded-xl p-4 text-center">
                <p className="text-dark-400 text-xs mb-1">Assessments</p>
                <p className="text-xl font-bold text-dark-50">{reportData.total_assessments}</p>
            ...
            {reportData.services_breakdown?.length > 0 && (
              <div>
                <h4 className="text-dark-50 font-medium mb-3">Services by Type</h4>
                <div className="grid grid-cols-2 gap-3">
            ...
            {reportData.weekly_trend?.length > 0 && (
              <div>
                <h4 className="text-dark-50 font-medium mb-3">Weekly Trend</h4>
                <div className="grid grid-cols-4 gap-3">
                  {reportData.weekly_trend.map((week: any, i: number) => (
                    <div key={i} className="bg-dark-700 rounded-xl p-3 text-center">
                      ...
                      <p className="text-xl font-bold text-dark-50">{week.assessments}</p>
                      <p className="text-dark-400 text-xs">{week.start_date}</p>
                    </div>
                  ))}
                </div>
              </div>
        ...
        return (
          <div className="p-6 space-y-6">
            ...
            <div className="bg-gradient-to-r from-accent-orange/10 to-yellow-500/10 rounded-xl p-4 border border-accent-orange/20">
              <h4 className="text-dark-50 font-bold">{reportData.period}</h4>
              <p className="text-dark-300 text-sm mt-1">{reportData.summary}</p>
            </div>
            ...
            {reportData.client_billing?.length > 0 && (
              <div>
                <h4 className="text-dark-50 font-medium mb-3">Client Billing</h4>
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  {reportData.client_billing.map((client: any, i: number) => (
                    <div key={i} className="flex items-center justify-between p-3 bg-dark-900/30 rounded-lg">
                      <div>
                        <p className="text-dark-50 text-sm font-medium">{client.client_name}</p>
                      ...
                      <div className="text-right">
                        ...
                        {client.estimated_weekly && (
                          <p className="text-dark-50 text-sm font-medium mt-1">${client.estimated_weekly.toFixed(0)}/wk</p>
                        )}
                      </div>
                    </div>
                  ))}
        ...
        return (
          <div className="p-6 space-y-6">
            ...
            <div className="bg-gradient-to-r from-accent-cyan/10 to-primary-500/10 rounded-xl p-4 border border-accent-cyan/20">
              <h4 className="text-dark-50 font-bold">Client Overview</h4>
              <p className="text-dark-300 text-sm mt-1">{reportData.summary}</p>
            </div>
            ...
            <div className="grid grid-cols-4 gap-3">
              <div className="bg-dark-700 rounded-xl p-4 text-center">
                ...
                <p className="text-xl font-bold text-dark-50">{reportData.total_clients}</p>
              </div>
              <div className="bg-dark-700 rounded-xl p-4 text-center">
            ...
            {reportData.clients?.length > 0 && (
              <div>
                <h4 className="text-dark-50 font-medium mb-3">All Clients</h4>
                <div className="space-y-2 max-h-80 overflow-y-auto">
                  {reportData.clients.map((client: any, i: number) => (
                    <div 
                      key={i} 
                      onClick={() => router.push('/clients')}
                      className="p-3 bg-dark-900/30 rounded-lg cursor-pointer hover:bg-dark-700/50 transition"
                    >
                      ...
                      {client.services_identified?.length > 0 && (
                        <div className="flex gap-1 flex-wrap mt-2 ml-11">
                          {client.services_identified.slice(0, 4).map((s: string, j: number) => (
                            <span key={j} className="px-2 py-0.5 bg-dark-700 rounded text-xs text-dark-300">{s}</span>
                          ))}
                        </div>
                      )}
                    </div>
      ...
      <main className="flex-1 p-8">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex justify-between items-start mb-8">
            <div>
              <h1 className="text-3xl font-bold text-dark-50 mb-2">Reports</h1>
              <p className="text-dark-400">Generate and export reports</p>
            </div>
          </div>
          ...
          <h2 className="text-xl font-semibold text-dark-50 mb-4">Generate Reports</h2>
                  ...
                  >
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        <h3 className="font-semibold text-dark-50 mb-1">{report.title}</h3>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className={`p-2 rounded-lg transition ${isExpanded ? 'bg-primary-50' : 'group-hover:bg-dark-700/30'}`}>
                          {isExpanded ? (
                            <ChevronDown className="w-5 h-5 text-primary-400" />
                          ) : (
                            <ChevronRight className="w-5 h-5 text-dark-400 group-hover:text-dark-50" />
                          )}
                        </div>
                      </div>
                    </div>
          ...
          <div className="card p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-dark-50">Weekly Activity</h3>
            </div>
            {(overviewStats?.assessments_this_week || 0) > 0 ? (
              ...
              <div className="h-64 flex flex-col items-center justify-center">
                ...
                <p className="text-dark-400 text-sm mt-1">Complete assessments to see your weekly activity</p>
              </div>
            )}
          </div>
          ...
          <div className="card p-6 mt-6">
            <h3 className="text-lg font-semibold text-dark-50 mb-4">Recent Activity</h3>
            {(overviewStats?.assessments_this_week || 0) === 0 ? (
              <div className="text-center py-8">
                <FileText className="w-12 h-12 text-slate-300 mx-auto mb-3" />
                <p className="text-dark-400">No recent activity</p>
                <p className="text-dark-400 text-sm mt-1">Complete an assessment to see activity here</p>
              </div>
            ) : (
