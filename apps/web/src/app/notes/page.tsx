    ...
    <div className="flex min-h-screen bg-dark-900">
      ...
      <main className="flex-1 min-w-0 flex flex-col">
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="w-80 xl:w-96 border-r border-dark-700/50 flex flex-col bg-dark-850 shrink-0">
            {/* List header */}
            <div className="p-4 border-b border-dark-700/50 space-y-3">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-2 bg-dark-700 border border-dark-600 rounded-lg text-white text-sm placeholder-dark-400 focus:outline-none focus:border-primary-500"
                />
              </div>
                  ...
                  <button
                    ...
                    className={`px-2 py-0.5 rounded-full text-[10px] font-medium transition-colors ${
                      tagFilter === tag
                        ...
                        : 'bg-dark-700/50 text-dark-400 border border-dark-600 hover:text-white'
                    }`}
                  >
                    {tag}
                  </button>
                    ...
                    <button
                      ...
                      className={`w-full text-left px-4 py-3 hover:bg-dark-700/30 transition-colors border-l-[3px] ${
                        selectedNote?.id === note.id
                          ? `bg-dark-700/40 ${NOTE_COLORS[note.color] || NOTE_COLORS.default}`
                      ...
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0 flex-1">
                          ...
                          {!note.ai_summary && note.content && (
                            <p className="text-xs text-dark-500 mt-0.5 line-clamp-1">{note.content.slice(0, 100)}</p>
                          )}
                        </div>
                        <span className="text-[10px] text-dark-500 shrink-0 mt-0.5">
                      ...
                      {note.tags && note.tags.length > 0 && (
                        <div className="flex gap-1 mt-1.5">
                          {note.tags.slice(0, 3).map(tag => (
                            <span key={tag} className="text-[9px] px-1.5 py-0.5 bg-dark-700/60 text-dark-400 rounded">
                              {tag}
                            </span>
                          ))}
                        </div>
          ...
          <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
            {!selectedNote ? (
              ...
              <>
                {/* Editor toolbar */}
                <div className="flex items-center justify-between px-6 py-3 border-b border-dark-700/50 bg-dark-800/30 shrink-0">
                  <div className="flex items-center gap-2">
                    ...
                    {selectedNote.source !== 'manual' && (
                      <span className="text-[10px] px-1.5 py-0.5 bg-dark-700/60 text-dark-400 rounded">{selectedNote.source}</span>
                    )}
                  </div>
                  <div className="flex items-center gap-1.5">
                    <button
                    ...
                    <button onClick={handleTogglePin} className="p-1.5 text-dark-400 hover:text-amber-400 transition-colors rounded-lg hover:bg-dark-700/50" title={selectedNote.is_pinned ? 'Unpin' : 'Pin'}>
                      {selectedNote.is_pinned ? <PinOff className="w-4 h-4" /> : <Pin className="w-4 h-4" />}
                    </button>
                    <div className="relative">
                      <button onClick={() => setShowTagPicker(!showTagPicker)} className="p-1.5 text-dark-400 hover:text-white transition-colors rounded-lg hover:bg-dark-700/50" title="Tags">
                        <Tag className="w-4 h-4" />
                      </button>
                      {showTagPicker && (
                        <div className="absolute right-0 top-full mt-1 bg-dark-800 border border-dark-600 rounded-lg shadow-xl p-2 z-20 min-w-[140px]">
                          {TAG_OPTIONS.map(tag => (
                            <button
                              key={tag}
                              onClick={() => handleToggleTag(tag)}
                              className={`w-full text-left px-2 py-1.5 text-xs rounded transition-colors flex items-center gap-2 ${
                                editTags.includes(tag) ? 'text-primary-400 bg-primary-500/10' : 'text-dark-300 hover:bg-dark-700'
                              }`}
                            >
                    ...
                    <button onClick={handleDeleteNote} className="p-1.5 text-dark-400 hover:text-red-400 transition-colors rounded-lg hover:bg-red-500/10" title="Delete note">
                    ...
                    <div className="border-t border-dark-700/50 pt-4">
                      <div className="flex items-center justify-between mb-3">
                        <h3 className="text-sm font-semibold text-white flex items-center gap-2">
                          ...
                          Tasks
                          {tasks.length > 0 && (
                            <span className="text-[10px] px-1.5 py-0.5 bg-dark-700 text-dark-400 rounded-full">
                      ...
                      {/* Add task form */}
                      {showAddTask && (
                        <div className="bg-dark-800/50 border border-dark-600 rounded-lg p-3 mb-3 space-y-2">
                          <input
                            type="text"
                            value={newTaskTitle}
                          ...
                          />
                          <div className="flex items-center gap-2">
                            <select value={newTaskPriority} onChange={e => setNewTaskPriority(e.target.value)} className="px-2 py-1.5 bg-dark-700 border border-dark-600 rounded text-xs text-white">
                              <option value="low">Low</option>
                              <option value="medium">Medium</option>
                              <option value="high">High</option>
                              <option value="urgent">Urgent</option>
                            </select>
                            <input type="date" value={newTaskDue} onChange={e => setNewTaskDue(e.target.value)} className="px-2 py-1.5 bg-dark-700 border border-dark-600 rounded text-xs text-white" />
                            <div className="flex-1" />
                            <button onClick={() => setShowAddTask(false)} className="px-2 py-1 text-xs text-dark-400 hover:text-white">Cancel</button>
                      ...
                      <div className="space-y-1">
                            ...
                            <div key={task.id} className={`flex items-center gap-2.5 px-3 py-2 rounded-lg hover:bg-dark-700/20 transition-colors group ${isDone ? 'opacity-50' : ''}`}>
                              <button onClick={() => !isDone && handleCompleteTask(task.id)} className="shrink-0">
                                {isDone ? (
                                  ...
                                  <Circle className="w-4 h-4 text-dark-500 hover:text-primary-400 transition-colors" />
                                )}
                              </button>
                              <div className="flex-1 min-w-0">
                                <p className={`text-xs ${isDone ? 'line-through text-dark-500' : 'text-white'}`}>{task.title}</p>
                                <div className="flex items-center gap-2 mt-0.5">
                                  ...
                                  {task.due_date && (
                                    <span className={`text-[9px] flex items-center gap-0.5 ${
                                      ...
                                      dueStatus === 'today' ? 'text-amber-400' : 'text-dark-500'
                                    }`}>
                              ...
                              <button onClick={() => handleDeleteTask(task.id)} className="opacity-0 group-hover:opacity-100 p-1 text-dark-500 hover:text-red-400 transition-all">
                        ...
                        {tasks.length === 0 && !showAddTask && (
                          <p className="text-xs text-dark-500 py-2 px-3">No tasks linked to this note</p>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="border-t border-dark-700/50 pt-4">
                      ...
                      {showAddReminder && (
                        <div className="bg-dark-800/50 border border-dark-600 rounded-lg p-3 mb-3 space-y-2">
                          ...
                          />
                          <div className="flex items-center gap-2">
                            <input type="datetime-local" value={newReminderAt} onChange={e => setNewReminderAt(e.target.value)} className="px-2 py-1.5 bg-dark-700 border border-dark-600 rounded text-xs text-white flex-1" />
                            <button onClick={() => setShowAddReminder(false)} className="px-2 py-1 text-xs text-dark-400 hover:text-white">Cancel</button>
                      ...
                      <div className="space-y-1">
                        {reminders.map(rem => {
                          ...
                          return (
                            <div key={rem.id} className={`flex items-center gap-2.5 px-3 py-2 rounded-lg hover:bg-dark-700/20 transition-colors group ${rem.is_dismissed ? 'opacity-40' : ''}`}>
                              <Bell className={`w-4 h-4 shrink-0 ${isPast && !rem.is_dismissed ? 'text-amber-400' : 'text-dark-500'}`} />
                              <div className="flex-1 min-w-0">
                                <p className="text-xs text-white">{rem.title}</p>
                                <p className={`text-[10px] ${isPast && !rem.is_dismissed ? 'text-amber-400' : 'text-dark-500'}`}>
                                  {new Date(rem.remind_at).toLocaleString()}
                                </p>
                              </div>
                              {!rem.is_dismissed && (
                                <button onClick={() => handleDismissReminder(rem.id)} className="opacity-0 group-hover:opacity-100 p-1 text-dark-500 hover:text-amber-400 transition-all" title="Dismiss">
                        ...
                        {reminders.length === 0 && !showAddReminder && (
                          <p className="text-xs text-dark-500 py-2 px-3">No reminders for this note</p>
                        )}
                      </div>
                    </div>
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-dark-800 border border-dark-600 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-dark-700">
              ...
              <button onClick={() => setShowNewNote(false)} className="p-1.5 text-dark-400 hover:text-white hover:bg-dark-700 rounded-lg transition-colors"><X className="w-4 h-4" /></button>
            </div>
            <div className="p-5">
              <input
                type="text"
            ...
            <div className="flex items-center gap-3 px-5 py-4 border-t border-dark-700">
              <button onClick={() => setShowNewNote(false)} className="flex-1 px-4 py-2 text-sm text-dark-300 hover:text-white bg-dark-700 hover:bg-dark-600 rounded-lg transition-colors">Cancel</button>
              <button onClick={handleCreateNote} disabled={!newTitle.trim()} className="flex-1 px-4 py-2 text-sm font-medium bg-primary-500 hover:bg-primary-600 disabled:bg-dark-600 disabled:text-dark-400 text-white rounded-lg transition-colors">
                Create
              </button>
            </div>
          </div>
